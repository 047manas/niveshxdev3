...
const CompanyOnboarding = () => {
    const steps = ['userAccount', 'otpVerification', 'companyProfile', 'companyVerifyPrompt', 'companyDetails', 'fundingHistory', 'companyContact', 'success'];
    ...
    const handleOtpSubmit = async (data) => {
        setLoading(true); setError('');
        try {
            const res = await fetch('/api/auth/verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: onboardingData.user.email, userOtp: data.otp }) });
            if (!res.ok) { const result = await res.json(); throw new Error(result.error || 'Failed to verify OTP.'); }
    ...
    const handleCompanyProfileSubmit = async (data) => {
        ...
        try {
            const res = await fetch('/api/company/onboard', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ step: 'checkCompany', email: updatedData.user.email, companyWebsite: data.companyWebsite }) });
            const result = await res.json();
            if (!res.ok) throw new Error(result.error || 'Failed to check company.');

            switch(result.status) {
                case 'LINKED_TO_VERIFIED_COMPANY': setSuccessMessage(`You have been successfully linked to ${data.companyName}.`); moveStep('success'); break;
                case 'COMPANY_EXISTS_UNVERIFIED': setOnboardingData(d => ({ ...d, companyId: result.companyId })); moveStep('companyVerifyPrompt'); break;
                case 'COMPANY_DOES_NOT_EXIST': moveStep('companyDetails'); break;
                default: throw new Error('Received an unexpected response from the server.');
            }
        } catch (err) { setError(err.message); } finally { setLoading(false); }
    };
    ...
    const handleCompanyDetailsSubmit = async (data) => {
        ...
        setOnboardingData(prev => ({ ...prev, company: { ...prev.company, ...details } }));
        moveStep('fundingHistory');
    };

    const handleFundingHistorySubmit = async (data) => {
        setOnboardingData(prev => ({ ...prev, company: { ...prev.company, funding: data } }));
        moveStep('companyContact');
    };

    const handleCompanyContactSubmit = async (data) => {
        setLoading(true); setError('');
        const finalData = { ...onboardingData, company: { ...onboardingData.company, ...data } };
        setOnboardingData(finalData);
        try {
            const res = await fetch('/api/company/onboard', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ step: 'createCompany', email: finalData.user.email, companyData: finalData.company }) });
            const result = await res.json();
            if (!res.ok) throw new Error(result.error || 'Failed to create company profile.');
            setSuccessMessage("Your company profile has been created and is pending verification.");
            moveStep('success');
        } catch (err) { setError(err.message); } finally { setLoading(false); }
    };
