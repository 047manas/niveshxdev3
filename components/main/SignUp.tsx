...
import { Control, FieldErrors, UseFormRegister, UseFormSetValue, FieldError } from 'react-hook-form';
...
interface SignUpProps {
  ...
  setUserType: (type: 'company' | 'investor') => void;
}

const SignUp = ({ setCurrentView, userType, setUserType }: SignUpProps) => {
  ...
  const [formData, setFormData] = useState<{ email: string; [key: string]: any } | null>(null);
  ...
  const handleInvestorOtpSubmit = async (data: { otp: string }) => {
    setLoading(true);
    setError('');
    try {
                ...
                <div className="space-y-6">
                    ...
                    <form onSubmit={investorForm.handleSubmit(handleInvestorSubmit)} className="space-y-4">
                        ...
                        <div className="flex justify-between pt-4 flex-col space-y-4">
                            {investorStep === 2 && (<div className="flex items-center space-x-2"><Checkbox id="investor-terms" onCheckedChange={(checked) => setInvestorAgreed(checked === true)} /><Label htmlFor="investor-terms" className="text-sm font-normal text-gray-400">I have read and agree to the <Link href="/terms-and-conditions.html" target="_blank" className="underline hover:text-primary">Terms and Conditions</Link> and <Link href="/privacy-policy.html" target="_blank" className="underline hover:text-primary">Privacy Policy</Link>.</Label></div>)}
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-900 p-4">
        <Card className="w-full max-w-4xl bg-gray-800 text-white border-gray-700">
            ...
            <CardContent className="p-4 sm:p-6">
                 <Tabs value={userType} onValueChange={(val) => setUserType(val as 'company' | 'investor')} className="w-full">
...
const InvestorStep1 = ({ control, register, errors, onEmailBlur, emailValidation }: InvestorStep1Props) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        <div className="space-y-2"><Label>First Name</Label><Input {...register("firstName")} className="bg-gray-700" />{errors.firstName && <p className="text-red-500 text-xs">{(errors.firstName as FieldError)?.message}</p>}</div>
        <div className="space-y-2"><Label>Last Name</Label><Input {...register("lastName")} className="bg-gray-700" />{errors.lastName && <p className="text-red-500 text-xs">{(errors.lastName as FieldError)?.message}</p>}</div>
